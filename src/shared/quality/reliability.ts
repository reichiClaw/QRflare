...
export interface ReliabilityInput {
  ...
  payloadBytes: number;
  payloadChars: number;
  /** True when the user pinned a QR version manually. */
...
export function evaluateReliability(input: ReliabilityInput): ReliabilityReport {
  ...
  if (background) {
    ...
    if (contrast < 2.5) {
      add(
        'contrast-critical',
        'critical',
        `Contrast ratio is only ${contrast.toFixed(1)}:1.`,
        'Use a much darker foreground on a light background.',
      );
    } else if (contrast < 4) {
      add(
        'contrast-low',
        'warning',
        `Contrast ratio ${contrast.toFixed(1)}:1 is below the recommended 4:1.`,
        'Darken the foreground or lighten the background.',
      );
    }
    const bgLum = relativeLuminance(parseHex(background) ?? { r: 255, g: 255, b: 255, a: 1 });
    const fgLum = Math.max(
      ...colors.map((c) => relativeLuminance(parseHex(c) ?? { r: 0, g: 0, b: 0, a: 1 })),
    );
    if (fgLum > bgLum) {
      add(
        'inverted',
        ...
        'Some scanners only read dark-on-light codes. Consider swapping the colours.',
      );
    }
  } else {
    add(
      'transparent',
      'warning',
      'Transparent background: scannability depends on where the image is placed.',
      'Place the code on a plain light surface, or disable transparency.',
    );
  }
  ...
  if (style.gradient.enabled) {
    if (style.gradient.stops.length > 3) {
      add(
        'gradient-complex',
        'warning',
        'Gradients with many colour stops lower contrast in places.',
        'Keep gradients to two or three dark colours.',
      );
    } else {
  ...
  if (style.finderColors.enabled && background) {
    const finderContrast = Math.min(
      contrastRatio(style.finderColors.frame, background),
      contrastRatio(style.finderColors.center, background),
    );
    if (finderContrast < 3) {
      add(
        'finder-contrast',
        'critical',
        'Finder pattern colours are too light against the background.',
        'Finder patterns must stay clearly darker than the background.',
      );
    }
    const moduleColor = style.gradient.enabled
      ? (style.gradient.stops[0]?.color ?? style.foreground)
      : style.foreground;
    if (contrastRatio(style.finderColors.frame, moduleColor) > 3) {
      add(
        'finder-differs',
        'warning',
        'Finder patterns differ strongly from the data modules.',
        'Some decoders threshold on a single colour; keep finder colours similar in darkness.',
      );
    }
  }
  ...
  if (style.moduleScale < 0.8) {
    add(
      'module-scale',
      'warning',
      `Modules are drawn at ${Math.round(style.moduleScale * 100)} % of their cell.`,
      'Very small modules reduce the dark area a scanner can detect.',
    );
  }
  if (style.moduleShape === 'diamond') {
    add(
      'diamond',
      'info',
      'Diamond modules cover only half of each cell.',
      'Keep the output large and the error correction high.',
    );
  }
  if (style.moduleShape === 'dots' && style.moduleScale < 0.9) {
    add(
      'dots-small',
      'warning',
      'Small dots are hard to read at print sizes.',
      'Increase module scale or output size.',
    );
  }
  ...
  if (quiet === 0) {
    add(
      ...
      'critical',
      'No quiet zone.',
      'Scanners need a blank margin of at least 4 modules.',
    );
  } else if (quiet < 4) {
  ...
  if (encode) {
    ...
    if (modulePixels < 2) {
      add(
        'size-critical',
        'critical',
        `Each module is only ${modulePixels.toFixed(1)} px at this output size.`,
        'Increase the export size or reduce the content.',
      );
    } else if (modulePixels < 4) {
      add(
        'size-small',
        'warning',
        `Modules are ${modulePixels.toFixed(1)} px – small for reliable scanning.`,
        'Export at a larger size (at least 4 px per module).',
      );
    }
    if (qrPixels < input.outputSize * 0.6) {
      add(
        'decor-heavy',
        'info',
        'Borders, frame and caption take more than 40 % of the image.',
        'The QR itself is small; export at a larger size.',
      );
    }
    ...
    if (style.logo.enabled && style.logo.dataUrl) {
      ...
      if (cover > recovery) {
        add(
          'logo-too-large',
          'critical',
          `The logo hides ${Math.round(cover * 100)} % of the modules, more than level ${encode.errorCorrection} can recover (${Math.round(recovery * 100)} %).`,
          'Reduce the logo scale or raise error correction to H.',
        );
      } else if (cover > recovery * 0.6) {
        add(
          'logo-large',
          'warning',
          `The logo hides ${Math.round(cover * 100)} % of the modules.`,
          'Keep logos under ~60 % of the recoverable area.',
        );
      }
      if (encode.errorCorrection !== 'H') {
        add(
          'logo-ec',
          'warning',
          `Error correction ${encode.errorCorrection} with a logo.`,
          'Use level H whenever a logo covers modules.',
        );
      }
      ...
      if (!style.logo.backplate.enabled && !style.logo.clearModules) {
        add(
          'logo-overlay',
          'warning',
          'The logo is drawn over live modules without a backplate.',
          'Enable the backplate or "clear modules" for a clean logo area.',
        );
      }
    }

    if (encode.byteLength > encode.maxBytesAtLevel * 0.9) {
      add(
        'capacity-limit',
        'warning',
        `The payload uses ${Math.round((encode.byteLength / encode.maxBytesAtLevel) * 100)} % of the maximum QR capacity at level ${encode.requestedErrorCorrection}.`,
        'Consider a short URL that redirects to the full content.',
      );
    } else if (input.forcedVersion && encode.usagePercent > 95) {
      add(
        'capacity-near',
        'info',
        `Payload uses ${encode.usagePercent} % of version ${encode.version}.`,
        'Adding more content will no longer fit this fixed version.',
      );
    }
    if (encode.version > 25) {
      add(
        'dense',
        'info',
        `Version ${encode.version} is very dense (${encode.matrix.size}×${encode.matrix.size}).`,
        'Print at a large size and use a shorter payload if possible.',
      );
    }
  }
